...
```sh
$ kubectl run test-$RANDOM --namespace=dev --rm -i -t --image=public.ecr.aws/docker/library/alpine:latest -- sh
...
```sh
$ kubectl run test-$RANDOM --namespace=prod --rm -i -t --image=public.ecr.aws/docker/library/alpine:latest -- sh
If you don't see a command prompt, try pressing enter.
/ # wget -qO- --timeout=2 http://web.default
<!DOCTYPE html>
<html>
