...
```sh
$ kubectl run test-$RANDOM --labels="app=inventory,role=web" --rm -i -t --image=public.ecr.aws/docker/library/alpine:latest -- sh
...
```sh
$ kubectl run test-$RANDOM --labels="app=other" --rm -i -t --image=public.ecr.aws/docker/library/alpine:latest -- sh
