...
```sh
$ kubectl run web --namespace=default --image=public.ecr.aws/docker/library/nginx:latest --labels="app=web" --expose --port=80
```
...
$ kubectl run test-$RANDOM --namespace=foo --rm -i -t --image=public.ecr.aws/docker/library/alpine:latest -- sh
...
```sh
$ kubectl run test-$RANDOM --namespace=default --rm -i -t --image=public.ecr.aws/docker/library/alpine:latest -- sh
/ # wget -qO- --timeout=2 http://web.default
<!DOCTYPE html>
<html>
```
